#!/usr/bin/env python3
# This file provides the 'summarize batch results' functionality for the LoRa Lite PHY toolkit.
...
# Executes the statement `RESULTS_JSON = ROOT / 'results' / 'batch_compat_results.json'`.
RESULTS_JSON = ROOT / 'results' / 'batch_compat_results.json'
# Executes the statement `CSV_OUT = ROOT / 'results' / 'batch_compat_summary.csv'`.
CSV_OUT = ROOT / 'results' / 'batch_compat_summary.csv'
# Executes the statement `MD_OUT = ROOT / 'results' / 'batch_compat_summary.md'`.
...
def main() -> None:
    ...
    for item in data.get('results', []):
        ...
        # Executes the statement `cpp = item.get('cpp_batch', {})`.
