...
constexpr size_t MAX_RAW_SAMPLES = 16 * 1024 * 1024; // for large files, increase if needed
...
// Ring buffer without allocations (only indices)
struct Ring {
    cfloat  buf[MAX_RAW_SAMPLES];
    ...
    size_t capacity() const { return MAX_RAW_SAMPLES; }

    // Span access without allocation/copy - with bounds checking
    const cfloat* ptr(size_t raw_idx) const {
        return (raw_idx < MAX_RAW_SAMPLES) ? &buf[raw_idx] : nullptr;
    }
    ...
    bool can_write(size_t amount) const { return tail + amount <= MAX_RAW_SAMPLES; }
};
...
struct Scheduler {
    ...
    void init(const RxConfig& c) {
        cfg = c;
        H_raw = dec_syms_to_raw_samples(2, cfg); // history 2 symbols
        W_raw = dec_syms_to_raw_samples(32, cfg);// search window (adapt per preamble+sync+header)
    ...
    bool step(Ring& r) {
        ...
        if (!raw) {
            DEBUGF("Invalid read pointer at win_head %zu", win_head);
            return false;
        }
