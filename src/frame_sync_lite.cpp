...
FrameSyncResult FrameSyncLite::process_samples(const std::vector<std::complex<float>>& input) {
    ...
    switch (m_state) {
        case FrameSyncState::DETECT: {
            ...
            if (bin_idx == -1) {
                // First symbol - accept any valid detection
                consecutive = (bin_idx_new != -1);
            } else {
                // Check for consecutive symbols with similar values (within ±1)
                int diff = std::abs(bin_idx_new - bin_idx);
                consecutive = (diff <= 1 || diff >= (static_cast<int>(m_number_of_bins) - 1)) && bin_idx_new != -1;
            }
