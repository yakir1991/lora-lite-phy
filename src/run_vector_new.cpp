...
int main(int argc, char** argv){
    if(argc < 2){ std::cerr << "Usage: run_vector <file> [sf] [cr] [crc(0/1)] [implicit(0/1)] [ldro]\n"; return 1; }
    ...
    params.sync_words = {0x12, 0x34}; // Default sync words
    
    ReceiverLite receiver(params);
    
    // Process samples in chunks
    const size_t chunk_size = (1u << sf) * params.oversample * 32; // Process ~32 symbols at a time
    size_t processed = 0;
