#include "whitening.hpp"
#include <array>

namespace lora_lite {
namespace {
constexpr std::array<uint8_t, 255> kWhiteningSeq = {
    
};
}

void whiten_in_place(uint8_t* data, size_t len, uint8_t /*seed*/) {
    if (!data) return;
    for (size_t i = 0; i < len; ++i) {
        data[i] = static_cast<uint8_t>(data[i] ^ kWhiteningSeq[i % kWhiteningSeq.size()]);
    }
}

void dewhiten_in_place(uint8_t* data, size_t len, uint8_t seed) {
    whiten_in_place(data, len, seed);
}

std::vector<uint8_t> whiten(const std::vector<uint8_t>& in, uint8_t seed) {
    std::vector<uint8_t> v = in;
    whiten_in_place(v.data(), v.size(), seed);
    return v;
}
std::vector<uint8_t> dewhiten(const std::vector<uint8_t>& in, uint8_t seed) {
    std::vector<uint8_t> v = in;
    dewhiten_in_place(v.data(), v.size(), seed);
    return v;
}

std::vector<uint8_t> dewhiten_prefix(const std::vector<uint8_t>& in, size_t prefix_len) {
    std::vector<uint8_t> v = in;
    size_t limit = std::min(prefix_len, v.size());
    for (size_t i = 0; i < limit; ++i) {
        v[i] = static_cast<uint8_t>(v[i] ^ kWhiteningSeq[i % kWhiteningSeq.size()]);
    }
    return v;
}

} // namespace lora_lite
