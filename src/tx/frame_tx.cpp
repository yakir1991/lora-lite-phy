...
std::span<const std::complex<float>> frame_tx(Workspace& ws,
    ...
    // Build frame bytes: header (with CRC) + payload + payload CRC
    auto hdr_bytes = lora::rx::make_local_header_with_crc(hdr);
    ...
    // Whitening over the entire frame
    auto lfsr = lora::utils::LfsrWhitening::pn9_default();
    lfsr.apply(frame.data(), frame.size());

    // Prepare buffers
    size_t bit_count = frame.size() * 2 * cr_plus4;
    uint32_t block_bits = sf * cr_plus4;
    if (bit_count % block_bits)
        bit_count = ((bit_count / block_bits) + 1) * block_bits;
    size_t nsym = bit_count / sf;
    ws.ensure_tx_buffers(frame.size(), sf, cr_plus4);

    auto& bits = ws.tx_bits;
    size_t bit_idx = 0;
    static lora::utils::HammingTables T = lora::utils::make_hamming_tables();
    auto emit_encoded = [&](uint8_t b){
        uint8_t n1 = b & 0x0F;
        uint8_t n2 = b >> 4;
    ...
    for (uint8_t b : frame) emit_encoded(b);
    while (bit_idx < bit_count) bits[bit_idx++] = 0;

    // Interleave
    const auto& M = ws.get_interleaver(sf, cr_plus4);
    auto& inter = ws.tx_inter;
    for (size_t off = 0; off < bit_count; off += M.n_in)
