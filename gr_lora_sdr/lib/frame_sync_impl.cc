...
namespace gr
{
    namespace lora_sdr
    {
        ...
        namespace
        {
            std::ofstream& cfo_debug_stream()
            {
                static std::ofstream stream;
                static bool initialized = false;
                if (!initialized) {
                    initialized = true;
                    if (const char* path = std::getenv("GRLORA_CFO_DEBUG")) {
                                          ...
                                          gr_vector_void_star &output_items)
        {
            ...
            case DETECT:
            {
                ...
                if (symbol_cnt == (int)(m_n_up_req))
                {
                    ...
                    auto& dbg = cfo_debug_stream();
                    if (dbg) {
                        dbg << "[frame_sync] sf=" << static_cast<int>(m_sf)
                            << " k_hat=" << k_hat
                            << " upchirps=" << preamb_up_vals.size()
                            << std::endl;
                    }
            ...
            case SYNC:
            {
                ...
                case DOWNCHIRP2:
                {
                    ...
                    auto& dbg_down = cfo_debug_stream();
                    if (dbg_down) {
                        dbg_down << "[frame_sync] sf=" << static_cast<int>(m_sf)
                                 << " down_val=" << down_val
                                 << " cfo_frac=" << m_cfo_frac
                                 << " sto_frac=" << m_sto_frac
                                 << std::endl;
                ...
                case QUARTER_DOWN:
                {
                    ...
                    auto& dbg_int = cfo_debug_stream();
                    if (dbg_int) {
                        dbg_int << "[frame_sync] sf=" << static_cast<int>(m_sf)
                                << " cfo_int=" << m_cfo_int
                                << " down_val=" << down_val
                                << std::endl;
                    }
